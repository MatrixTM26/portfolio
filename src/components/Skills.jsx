...
function Chip({ item }) {
  return (
    <div className="skill-chip">
      {item.img
        ? <img src={item.img} alt={item.name} width="16" height="16" loading="lazy" decoding="async" className={item.inv ? 'icon-invert' : ''} />
...
export default function Skills() {
  ...
  return (
    <section className="section skills" id="skills">
      <div className="skills-bg-layer" data-parallax="slow" aria-hidden="true" />
      <div className="container">
        ...
        <div className={`skill-categories-grid reveal${cats.visible ? ' visible' : ''}`} ref={cats.ref}>
          {CATEGORIES.map((cat, i) => (
            <div key={cat.name} className="skill-cat-card" style={{ transitionDelay: `${i * 80}ms` }}>
              <div className="skill-cat-top">
                <i className={`${cat.icon} skill-cat-icon`} aria-hidden="true" />
