...
export default function Contact() {
  ...
  return (
    <section className="section contact-section" id="contact">
      <div className="contact-bg-layer" data-parallax="slow" aria-hidden="true" />
      <div className="container">
        ...
        <div className={`reveal${header.visible ? ' visible' : ''}`} ref={header.ref}>
          ...
          <p className="section-desc">
            Open to security collaborations, bug bounty partnerships, and red team engagements.
          </p>
        </div>
            ...
            <div className="contact-channels">
              {CHANNELS.map((ch, i) => (
                <a key={ch.label} href={ch.href} target="_blank" rel="noopener noreferrer"
                  className="contact-channel" style={{ transitionDelay: `${i * 60}ms` }}>
                  <div className="ch-icon-wrap"><i className={ch.icon} aria-hidden="true" /></div>
                  <div className="ch-info">
                    <div className="ch-label">{ch.label}</div>
                    <div className="ch-value">{ch.value}</div>
                  </div>
                  <i className="fa-solid fa-arrow-right ch-arrow" aria-hidden="true" />
                </a>
              ))}
            </div>
          </div>

          <div className={`contact-form reveal-right${rightCol.visible ? ' visible' : ''}`} ref={rightCol.ref}>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label" htmlFor="c-name">Name</label>
                <input id="c-name" className="form-input" type="text" name="name"
                  placeholder="Your name" value={form.name} onChange={handleChange} autoComplete="name" />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="c-email">Email</label>
                <input id="c-email" className="form-input" type="email" name="email"
                  placeholder="your@email.com" value={form.email} onChange={handleChange} autoComplete="email" />
              </div>
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="c-subject">Subject</label>
              <input id="c-subject" className="form-input" type="text" name="subject"
            ...
            <div className="form-group">
              ...
              <textarea id="c-message" className="form-textarea" name="message"
                placeholder="Describe your project or question..."
                value={form.message} onChange={handleChange} rows={5} />
            </div>
            {status === 'success' && (
              <div className="form-status success" role="status">
                <i className="fa-solid fa-circle-check" aria-hidden="true" /> Message sent.
              </div>
            )}
            {status === 'error' && (
              <div className="form-status error" role="alert">
                <i className="fa-solid fa-circle-exclamation" aria-hidden="true" /> Fill in all required fields.
              </div>
            )}
